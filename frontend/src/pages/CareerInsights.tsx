...
const CareerInsights = () => {
  ...
  const fetchInsight = async () => {
    try {
      setLoading(true);
      const response = await getCareerInsights(selectedResume);

      setInsight(response.data);
    } catch (error) {
      console.log("No previous insight found. Generating...");

      try {
        const response = await generateCareerInsights(selectedResume);
        ...
        {loading ? (
          ...
          <>
            ...
            <section className="bg-white border rounded-xl p-8 shadow-sm text-center">
              <h2 className="text-2xl font-semibold mb-4">Career Level</h2>
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white border rounded-xl p-6 shadow-sm">
                <h2 className="text-2xl font-semibold mb-5">Strengths</h2>
              ...
              <div className="bg-white border rounded-xl p-6 shadow-sm">
                ...
                <ul className="space-y-3">
                  {insight.improvementAreas?.map((item, index) => (
                    <li
                      key={index}
            ...
            <section className="bg-white border rounded-xl p-6 shadow-sm">
              <h2 className="text-2xl font-semibold mb-6">Recommended Roles</h2>

              <div className="flex flex-wrap gap-3">
                {insight.recommendedRoles?.map((role, index) => (
                  <span
                    key={index}
            ...
            <section className="bg-white border rounded-xl p-6 shadow-sm">
              ...
              <div className="space-y-4">
                {insight.learningRecommendations?.map((item, index) => (
            ...
            <section className="bg-white border rounded-xl p-6 shadow-sm">
              <h2 className="text-2xl font-semibold mb-6">Career Roadmap</h2>

              <div className="space-y-4">
                {insight.roadmap?.map((step, index) => (
                  <div key={index} className="flex gap-4 items-start">
                    <div className="bg-violet-600 text-white w-8 h-8 rounded-full flex items-center justify-center">
                      {index + 1}
                    </div>

                    <div className="border rounded-lg p-4 flex-1">{step}</div>
                  </div>
                ))}
              </div>
            </section>
